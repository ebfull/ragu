...
* `enforce_zero` creates a [linear constraint](../design/circuits/constraints.md#linear-constraints) that enforces that a linear combination of wires must equal zero. This produces a new term in $Y^j$ for some unused $j$.
* `mul` creates new wires $(a, b, c)$ that must satisfy a [multiplication constraint](../design/circuits/constraints.md#multiplication-constraints) $ab = c$. This allocates (or assigns) the corresponding powers $(X^{2n + i}, X^{2n - 1 - i}, X^i)$ for some unused $i$.
