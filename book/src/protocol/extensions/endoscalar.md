...
Consider a random verifier challenge $\alpha\in\F_p$ produced in a circuit
over $\F_p$ where we want to compute $\alpha\cdot G\in\G_1$. Any group
operations inside an $\F_p$-circuit require expensive non-native
arithmetic, so we prefer deferring this to an $\F_q$-circuit where group
elements are natively represented and arithmetic over coordinates
$\in\F_q$ is also native. We can move $\alpha$ across circuits via an
endoscalar: first, run $\mathsf{extract}$ in the $\F_p$-circuit to obtain
the endoscalar as a public output; then use the same endoscalar as the
public input of the $\F_q$-circuit and constrain $\endo{s}\cdot G$
completely natively.
