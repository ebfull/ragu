...
//!
//! This computes the native `eval` stage witness containing the claimed
//! evaluations at $u$ of every element that was also queried in the `query`
//! stage. The evaluation $f(u)$ is derived from the aforementioned
//! evaluations.
...
impl<C: Cycle, R: Rank, const HEADER_SIZE: usize> Application<'_, C, R, HEADER_SIZE> {
    ...
    /// Samples a fresh blinding for the native `eval` stage and returns its
    /// rx polynomial.
    ///
    /// The caller stores the result on the builder; sampling is separated
    /// from `compute_eval` so that witness assembly stays deterministic.
