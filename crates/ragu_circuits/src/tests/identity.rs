...
fn test_root_identity() {
    ...
    assert_eq!(metrics.routines.len(), 2, "root + one routine = 2 records");

    assert!(
        matches!(metrics.routines[0].identity, RoutineIdentity::Root),
        "record 0 must be Root"
    );
    assert!(
        matches!(metrics.routines[1].identity, RoutineIdentity::Routine(_)),
